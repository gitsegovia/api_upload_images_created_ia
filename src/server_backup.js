...
app.post("/api/images/single", verifyToken, uploadSingle.single("image"), async (req, res) => {
    ...
    try {
        ...
        await sharp(req.file.path).jpeg({ quality: 80 }).toFile(`uploads/${filename}-original.jpg`)

        // crear la imagen de tamaño mediano
        await sharp(req.file.path).resize({ width: 640 }).jpeg({ quality: 80 }).toFile(`uploads/${filename}-medium.jpg`)

        // crear la imagen de tamaño thumbnail
        await sharp(req.file.path).resize({ width: 160 }).jpeg({ quality: 80 }).toFile(`uploads/${filename}-thumb.jpg`)
...
app.post("/api/images/multiple", verifyToken, uploadMultiple.array("image", 5), async (req, res) => {
    ...
    try {
        for (const file of req.files) {
            // crear la imagen original
            await sharp(file.path).jpeg({ quality: 80 }).toFile(`uploads/${file.filename}-original.jpg`)

            // crear la imagen de tamaño mediano
            await sharp(file.path).resize({ width: 640 }).jpeg({ quality: 80 }).toFile(`uploads/${file.filename}-medium.jpg`)

            // crear la imagen de tamaño thumbnail
            await sharp(file.path).resize({ width: 160 }).jpeg({ quality: 80 }).toFile(`uploads/${file.filename}-thumb.jpg`)
...
app.post("/api/documents/multiple", verifyToken, uploadMultiple.array("documents", 8), async (req, res) => {
    // Verificar si se ha enviado algún archivo
    if (!req.files || req.files.length === 0) {
        return res.status(400).send("No se han enviado archivos.")
    }
